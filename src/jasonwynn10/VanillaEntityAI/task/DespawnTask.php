...
class DespawnTask extends AsyncTask {
	/** @var string[][] $chunks */
	private $chunks = [];

	/**
	 * DespawnTask constructor.
	 */
	public function __construct() {
			...
			foreach($level->getPlayers() as $player) {
				foreach($player->usedChunks as $hash => $sent) {
					if($sent) {
						...
						$chunks[$level->getId()][$hash] = $player->getLevel()->getChunk($chunkX, $chunkZ, true);
					}
				}
			}
		...
		foreach($chunks as $levelId => $chunkArr)
			foreach($chunkArr as $chunk)
				$this->chunks[$levelId][] = $chunk->fastSerialize();
	}
